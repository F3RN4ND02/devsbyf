...
router.post(
	...
	async (req, res) => {
		...
		try {
			...
			console.error(err.message)
			res.status(500).send('Server error social')
		}
	}
)
...
router.get('/', async (req, res) => {
	try {
		...
		console.error(err.message)
		res.status(500).send('Server error all profiles')
	}
})
	...
	try {
		...
		if (!profile)
			return res.status(400).json({
				msg: 'This profile does not exists',
			})
		...
		res.status(500).send('Server error individual profile')
	}
})
...
router.put(
	...
	async (req, res) => {
		...
		try {
			...
			console.error(err.message)
			res.status(500).send('Server Error newExp')
		}
	}
)
...
router.put(
	...
	async (req, res) => {
		...
		try {
			...
			console.error(err.message)
			res.status(500).send('Server Error newEduc')
		}
	}
)
